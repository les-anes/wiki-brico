...
try {
  ...
  assert.equal(tutorials.length, 37);
  assert.equal(new Set(tutorials.map((t) => t.id)).size, 37);
  const home = render("/");
  ...
  assert.equal(countCards(render("/tutoriels/")), 37);
  assert.equal(
    countCards(
      render(
        catalogHref({
  ...
  console.log(
    "Accueil, catalogue, 37 fiches, 39 URLs, sitemap, robots, fil d’Ariane, 404, shim et hydratation : contrôles réussis.",
  );
} finally {
  await server.close();
}
