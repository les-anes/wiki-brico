...
export function CatalogPage({
  ...
  const filters = readCatalogFilters(hash);
  ...
  return (
    <main tabIndex={-1}>
      {" "}
      <section
        className="container tutorials-section catalog-page"
        id="tutoriels"
        tabIndex={-1}
      >
        ...
        <div className="filters">
          <div className="filter-tabs">
            <button
              className={category === "all" && !savedOnly && !query && difficulty === "all" && journey === "all" ? "active" : ""}
              onClick={() => {
          ...
          {filtered.map((t) => (
            <article className="tutorial-card" key={t.id}>
              <div className="card-image">
                <a href={`#tutoriel/${t.id}`} tabIndex={-1} aria-hidden="true">
